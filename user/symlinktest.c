...
static void testsymlink(void) {
  ...
  for (int i = 0; i < NINODE + 2; i++) {
    ...
    if (fd1 < 0) fail("open() failed in many test");
    char buf[16];
    buf[0] = '\0';
    if (read(fd1, buf, sizeof(buf)) != 1) fail("read() failed in many test");
    if (buf[0] != '#') fail("wrong content in many test");
    close(fd1);
    fd1 = -1;
  }

  unlink("/testsymlink/a");
  if (symlink("/README", "/testsymlink/a") != 0) fail("could not link to /README");
  fd1 = open("/testsymlink/a", O_RDONLY);
  if (fd1 < 0) fail("could not open symlink pointing to /README");
  close(fd1);
  fd1 = -1;
