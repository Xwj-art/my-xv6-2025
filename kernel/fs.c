...
static uint bmap(struct inode* ip, uint bn) {
  ...
  if (bn >= NINDIRECT * NINDIRECT) panic("bmap: out of range");
...
// 参数为二级索引块的块号，释放指定二级索引块指向的所有块
void helpfree(uint addr, uint dev) {
  struct buf* bp = bread(dev, addr);
  uint* psecond = (uint*)bp->data;

  for (int i = 0; i < NINDIRECT; i++)
    if (psecond[i]) bfree(dev, psecond[i]);
...
void itrunc(struct inode* ip) {
  ...
  if (ip->addrs[NDIRECT + 1]) {
    ...
    // 拿到一级索引块的内容
    a = (uint*)bp->data;
    // 我需要一个函数能帮我释放掉二级索引指向的块
    // 只需要一级间接索引进行一轮循环，函数内部再清理掉一级索引块指向的块
    // 就清空了全部索引块
    for (uint k = 0; k < NINDIRECT; k++)
      if (a[k]) helpfree(a[k], ip->dev);
    brelse(bp);
