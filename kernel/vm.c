...
void pg(char *s, pagetable_t pagetable) {
  pt(s);
  vmprint(pagetable);
}

// 如果pte_c是1，则需要调用这个函数来
// 分配内存，复制数据，减少引用计数。
...
int iscow(pagetable_t pagetable, uint64 va) {
  pte_t *pte = walk(pagetable, va, 0);
  if (!pte) return 0;
  if ((PTE_FLAGS(*pte) & PTE_V) && (PTE_C & PTE_FLAGS(*pte))) {
    return 1;
  }
  return 0;
}
...
int uvmcopy(pagetable_t old, pagetable_t new, uint64 sz) {
  ...
  for (i = 0; i < sz; i += PGSIZE) {
    ...
    if ((*pte & PTE_V) == 0) panic("uvmcopy:invalid");  // physical page hasn't been allocated
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    // 父子进程全都设为只读，使用pte_c位
    if (iscow(old, i)) flags = (flags & ~PTE_W) | PTE_C | PTE_R;
    *pte = (*pte & ~(0x1ff)) | flags;
    // 子进程页表映射
