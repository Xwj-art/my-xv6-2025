...
void e1000_init(uint32 *xregs) {
  ...
  initlock(&transmit_lock, "transmit");
...
int e1000_transmit(char *buf, int len) {
  // 拿到索引判断能否写入数据，若能说明上次已经完成通信，释放分配的缓冲区
  // pt("acquire1");
  acquire(&transmit_lock);
  // pt("acquire2");
  uint index = regs[E1000_TDT];
  if (tx_ring[index].status != E1000_TXD_STAT_DD) {
    release(&transmit_lock);
    return -1;
  }
