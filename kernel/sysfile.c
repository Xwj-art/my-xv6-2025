...
static struct inode* follow_symlink(struct inode* ip, int depth) {
  ...
  if (depth >= 10) {
    iunlockput(ip);  // 深度超过限制，释放传入的inode
    return 0;
  }

  if (ip->type != T_SYMLINK) {
    return ip;  // 不是符号链接，直接返回（调用者负责解锁）
  }

  // 读取符号链接内容
  int n = readi(ip, 0, (uint64)target, 0, MAXPATH);
  if (n <= 0 || n >= MAXPATH) {
    iunlockput(ip);  // 读取失败，释放传入的inode
    return 0;
  }
  target[n] = 0;
...
uint64 sys_open(void) {
  ...
  if (omode & O_CREATE) {
    ...
    // 若确实存在inode，判断dir和symlink
    ilock(ip);
    // 在 sys_open 中处理符号链接的部分：
    if (ip->type == T_SYMLINK && !(omode & O_NOFOLLOW)) {
      struct inode* new_ip = follow_symlink(ip, 0);
      if (new_ip == 0) {
        // follow_symlink 已经释放了传入的 ip，并返回 0
        end_op();
        return -1;
      }
      ip = new_ip;  // 使用最终的目标 inode
    }
